    ...
    <>
      <header>
        <div className="brand-title">
          ...
          <div>
            <h1>ControlPlane.ai Governance Console</h1>
            <p>Live, tamper-evident audit ledger stream & policy calibration engine</p>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <span className="status-badge">
            <span className="pulse-dot" /> Audit Ledger Active
          </span>
          <select value={tenant} onChange={(e) => setTenant(e.target.value)}>
            <option value="">All Tenant Policies</option>
            {tenants.map((t) => (
      ...
      {error && (
        <div className="error-banner">
          ⚠️ Could not connect to Audit Ledger API: {error}. Make sure <code>console/backend</code> is running on port 8001.
        </div>
      )}

      <SummaryCards summary={summary} />

      <MetricsCharts summary={summary} />

      {tenant && <RiskAppetiteControl tenant={tenant} />}
      {tenant && <HumanFeedbackPanel tenant={tenant} refreshKey={feedbackRefreshKey} />}

      <div className="panel">
        <h2>Decision Taxonomy Breakdown</h2>
        <DecisionBars counts={summary?.decision_counts ?? {}} />
      </div>

      <div className="panel">
        <h2>Audit Ledger Events & Requests</h2>
